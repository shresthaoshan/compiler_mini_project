...
def main():
	...
	# get productions input
	firstFollow.main()
	print("\tFIRST AND FOLLOW OF NON-TERMINALS")
	# loop through non-terminals and calculate their first/follow
	for nt in ntl:
		firstFollow.compute_first(nt)
		firstFollow.compute_follow(nt)
		print(nt)
		# print respective first/follow
	...
	print(nt_list)
	print(t_list)
	# calculate canonical items in the DFA
	j=calc_states()
	ctr=0
	for s in j:
		...
		for i in s:
			print("\t", i)
		ctr+=1
	# build table from canonical items of DFA
	table=make_table(j)
	columns = t_list + nt_list
	# printing table in formatted manner
	print("\n\tCLR(1) TABLE\n")
	df_table = pd.DataFrame([x for _, x in table.items()], columns=columns)
	df_clean = df_table.replace(to_replace=np.NAN, value=" ")
	print(df_clean.head(20))
	# checking if there are any shift/reduction conflicts or reduction/reduction conflicts
	sr, rr=0, 0
